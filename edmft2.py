...
for isc in range(nsc):
    ...
    if mpi.rank==0:
        f=open("K-omega"+str(isc)+".dat","w")
        for i in range(ndiv_tau):
            f.write("%i %f %f\n"%(i, matsubara_freq_boson[i], K_omega[i].real))
        f.close()

    if mpi.rank==0:
        f=open("K-"+str(isc)+".dat","w")
        for i in range(ndiv_tau+1):
            f.write("%i %f %f\n"%(i, K_tau[i], Kp_tau[i]))
        f.close()
    ...
    #occ=0.
    #for i in range(2):
        #rtmp=ar['/simulation/results/density_%i/mean/value'%i]
        #occ+=rtmp
        #print "n",i,"=",rtmp
